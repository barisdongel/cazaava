   ...
   <form action="../islem.php" method="POST" enctype="multipart/form-data">
     <div class="card-body">
      ...
      <div class="form-group">
          ...
          <img style="width: 30%;" src="../<?php echo $ayarcek['ayar_logo'] ?>">
          <input style="height: 30%;" type="file" name="ayar_logo" class="form-control">
      </div>
      <div class="form-group">
          <label><i class="fa fa-code"></i> Site Yazarı</label>
          <input type="text" name="ayar_author" value="<?php echo $ayarcek['ayar_author'] ?>" class="form-control">
      </div>
      <div class="form-group">
          <label><i class="fa fa-share-alt"></i> Site URL</label>
          <input type="url" name="ayar_siteurl" value="<?php echo $ayarcek['ayar_siteurl'] ?>" class="form-control">
      </div>
      <div class="form-group">
          <label><i class="fa fa-align-left"></i> Site Açıklaması</label>
          <input type="text" name="ayar_desc" value="<?php echo $ayarcek['ayar_desc'] ?>" class="form-control">
      </div>
      <div class="form-group">
          <label><i class="fa fa-key"></i> Anahtar Kelimeler</label>
          <input type="text" name="ayar_key" value="<?php echo $ayarcek['ayar_key'] ?>" class="form-control">
      </div>
      <div class="form-group">
          <label><i class="fa fa-heading"></i> Ana Sayfa Başlığı</label>
          <input type="text" name="ayar_baslik" value="<?php echo $ayarcek['ayar_baslik'] ?>" class="form-control">
      </div>
      <div class="form-group">
          <label><i class="fa fa-chevron-circle-down"></i> Footer Yazısı</label>
          <textarea rows="8" name="ayar_footer" class="form-control" id="ckeditor1"><?php echo $ayarcek['ayar_footer'] ?></textarea>
      </div>
      <hr>
      <h5><i class="fa fa-ghost"></i> API AYARLARI</h5>
      <div class="form-group">
          <label><i class="fa fa-heading"></i> Google Map Api</label>
          <textarea rows="5" type="text" name="ayar_googlemap" class="form-control"><?php echo $ayarcek['ayar_googlemap'] ?></textarea>
      </div>
  </div>
</div>
