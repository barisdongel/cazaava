...
if (isset($_POST['arama'])) {
	...
	$urunkategori=$db->prepare("SELECT * FROM urunler_tbl where urun_kategori=:urun_kategori");
	$urunkategori->execute(array('urun_kategori' => $_GET['urun_kategori']));
		...
		<div class="card">
			...
			<div class="card-header bg-primary justify-content-center">
				...
				<?php while ($kategorice1=$kategoriso1->fetch(PDO::FETCH_ASSOC)) { ?>
					...
					<td><a style="color:#fff;" href="urunkategoricek.php?urun_kategori=<?php echo $kategorice1['kategori_id'] ?>" class="ktgry btn p-3 text-center"><?php echo $kategorice1['kategori_ad'];?></a></td>
				<?php } ?>
			</div>
			<form action="" method="POST">
				<div class="input-group col-md-5 m-2">
			...
			<form action="../islem.php" method="POST">
				<table class="table table-striped table-md">
					...
					<?php while ($urunkategoricek=$urunkategori->fetch(PDO::FETCH_ASSOC)) { ?>
						<tr>
							<?php $uruncek=$urunsor->fetch(PDO::FETCH_ASSOC) ?>
							<td><img style="width: 100%;" src="../<?php echo $uruncek['urun_foto'] ?>" alt=""></td>
							<td style="width:30%;"><?php echo $urunkategoricek['urun_ad']; ?></td>
							<?php $kategoricek=$kategorisor->fetch(PDO::FETCH_ASSOC); ?>
							<td style="width:16%;"><?php echo $kategoricek['kategori_ad']; ?></td>
							<td><a href="urun-fotogaleri.php?urun_id=<?php echo $uruncek['urun_id'] ?>" class="btn btn-outline-warning"><i class="fa fa-images"></i> Fotogaleri</a></td>
							<td><a href="urun-duzenle.php?urun_id=<?php echo $uruncek['urun_id'] ?>" class="btn btn-outline-info"><i class="fa fa-pencil-alt"></i> Düzenle</a></td>
							<td><a href="../islem.php?urunsil=ok&urun_id=<?php echo $uruncek['urun_id'] ?>" onclick="return confirm('Silmek istediğinize emin misiniz?')" class="btn btn-outline-primary p-3"><i class="fa fa-trash"></i> Sil</a></td>
						</tr>
					<?php } ?>
				</table>
			</div>
